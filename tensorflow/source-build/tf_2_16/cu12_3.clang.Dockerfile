...
FROM curoky/infra-image:cuda12.3-cudnn9 as base_image_v2
...
RUN apt-get update -y \
  && apt-get install -y --no-install-recommends \
    curl clang-15 git python3.11 python3.11-dev python3-pip patchelf
...
ENV TF_CUDA_VERSION=12 \
  ...
  CC=/usr/bin/clang-15 CXX=/usr/bin/clang++-15 \
